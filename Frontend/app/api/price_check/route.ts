...
export async function POST(req: NextRequest, res: NextResponse) {
  ...
  const provider = new ethers.JsonRpcProvider(process.env.INFURA_BASE_URL + process.env.INFURA_API_KEY);
	...
	const available = await registrar.rentPrice(data['name'], data['duration']);
	// console.log(ethers.formatEther(available[0]));

	return NextResponse.json({"price": ethers.formatEther(available[0])}, {status:200});
}
